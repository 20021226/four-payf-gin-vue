...
// GetMerUser 根据id获取merUser表记录
// Author [yourname](https://github.com/yourname)
func (merUserService *MerUserService) GetNomalMerUser(ctx context.Context, params payReq.PayQrcodeParms) (list []example.MerUser, err error) {
	db := global.GVA_DB.WithContext(ctx).Model(&example.MerUser{})
	err = db.Where("id = ? AND mer_type = ? AND state = ? AND is_del = ?", params.MerId, params.MerType, 1, 0).Find(&list).Error
	return
}
